...
def test_calculate_freshness_boundaries(
    age_minutes: int,
    expected_freshness: CatalystFreshness
) -> None:
...
def test_calculate_news_age_minutes() -> None:
    ...
    published_at = current_time - timedelta(
        minutes=125
    )
...
def test_find_newest_catalyst_article_ignores_unknown_articles() -> None:
    ...
    catalyst_article = create_article(
        "Company reports quarterly earnings",
        current_time - timedelta(
            hours=8
        )
    ...
    unknown_article = create_article(
        "CEO discusses company culture",
        current_time - timedelta(
            minutes=30
        )
    )
