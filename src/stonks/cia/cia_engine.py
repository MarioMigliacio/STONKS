...
def collect_catalyst_categories(
    articles: list[NewsArticle], current_time: datetime
    ...
    for article in articles:
        ...
        news_age_minutes = calculate_news_age_minutes(article.published_at, current_time)
        ...
        if freshness in [
            ...
            add_unique_categories(active_categories, categories)

        else:
            add_unique_categories(historical_categories, categories)

    return (active_categories, historical_categories)


def calculate_average_sentiment(articles: list[NewsArticle]) -> float:
    """Calculate the average sentiment score across all articles."""

    if not articles:
        return 0.0

    sentiment_total = sum(article.sentiment_score for article in articles)

    return sentiment_total / len(articles)


def classify_overall_sentiment(average_sentiment: float) -> str:
...
def calculate_confidence(articles: list[NewsArticle]) -> float:
    """
    ...
    confidence = recognized_article_count / len(articles)
...
def build_summary(
    ...
    if active_categories:
        active_text = ", ".join(category.value for category in active_categories)
    ...
    return f"No significant catalyst intelligence was identified for {ticker}."
...
def build_catalyst_report(
    ...
    if newest_catalyst:
        news_age_minutes = calculate_news_age_minutes(newest_catalyst.published_at, current_time)
    ...
    return CatalystReport(
        ...
        has_breaking_news=(freshness == CatalystFreshness.BREAKING),
    )
